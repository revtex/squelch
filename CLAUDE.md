...
**Backend** (`backend/`, Go 1.26, Gin) follows golang-standards/project-layout:
- `cmd/server/main.go` wires everything; `cmd/migrate` is a separate migration binary.
- `internal/` holds all code (no public Go API). Each subpackage is one feature or cross-cutting concern: `audio` (FFmpeg worker pool), `auth` (JWT + refresh-token rotation), `ws` (WebSocket hub + protocol), `dirmonitor` (filesystem ingest watchers), `downstream`, `stream` (the server-side background audio stream), `trmqtt`, `secrets`, `safehttp` (hardened outbound HTTP client), `seed`, `db` (sqlc output + connection), etc. RadioReference lives under `handler/admin/radioreference/`, not as a top-level package.
